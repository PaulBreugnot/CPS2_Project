...
const char* wifi_ssid     = "Bbox-1EE87B";
...
/* if regular = true : a message is sent every DELTA_T ms 
   if regular = false: a message is sent if the value has changed */
...
double getHmdt()
{
  ...
  }else{sendValuesHMDT(hmdt);}
}
double getLumi()
{
  float lumi = analogRead(LUM) ;
  if (regular == false){
    if (lumi != oldlumi){
      oldlumi = lumi ;
      sendValuesLUMI(lumi) ;
    }
  }else{sendValuesHMDT(lumi);}
}
/* ################################################################### */
...
void sendValuesHMDT(float hmdt) {
  if ( mqtt_client.connected() ) {
    if ( hmdt != -1 ){
      Serial.print("hmdt : ");
      Serial.println(hmdt);
...
void sendValuesLUMI(float lumi) {
  if ( mqtt_client.connected() ) {
    if ( lumi != -1 ){
      Serial.print("lumi : ");
      Serial.println(lumi);
